...
import { LifecycleBinding } from '../lifecycle/lifecycle.binding';
...
export class ProviderBinding<T> {
   ...
   */
  public toClass(target: AbstractConstructor<T> | Constructor<T>): LifecycleBinding<T> {
   ...
   */
  public toFactory(factory: Factory<T>): void {
  ...
  public toSelf(): LifecycleBinding<T> {
    if (typeof this.binding.token !== 'function' || !('prototype' in this.binding.token)) {
