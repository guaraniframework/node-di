 ...
 */
export function LazyInjectAll<T>(wrappedToken: () => InjectableToken<T>): PropertyDecorator & ParameterDecorator {
  return function (
    ...
    if (typeof propertyKey === 'undefined' && typeof parameterIndex !== 'undefined' && typeof target !== 'object') {
      setTokenDescriptor<T>(PARAM_TOKENS, target, parameterIndex, { token: lazyToken, multiple: true });
    }

    // Injects into a property of the target.
    if (typeof propertyKey !== 'undefined' && typeof parameterIndex === 'undefined') {
      setTokenDescriptor<T>(PROP_TOKENS, target, propertyKey, { token: lazyToken, multiple: true });
    }
  };
}
