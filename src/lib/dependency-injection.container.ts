import { Binding } from './bindings/binding/binding';
...
import { isClassProvider } from './providers/class/class.provider';
import { isFactoryProvider } from './providers/factory/factory.provider';
import { Provider } from './providers/provider';
import { isTokenProvider } from './providers/token/token.provider';
import { isValueProvider } from './providers/value/value.provider';
...
export class DependencyInjectionContainer {
  ...
  private resolveConstructorDependencies<T>(
    ...
    const paramTypeDescriptors: TokenDescriptor<unknown>[] | undefined = Reflect.getMetadata(PARAM_TYPES, class_);

    if (!Array.isArray(paramTypeDescriptors)) {
      throw new InvalidProviderError(class_);
    }
 ...
 * ```
 */
export const Container: DependencyInjectionContainer = new DependencyInjectionContainer();
